...
7. [Sugestões para o Time de Desenvolvimento](#7-sugestões-para-o-time-de-desenvolvimento)
...
# 5. Gerar relatório HTML (Mochawesome)
npm run test:report
```
...
| `cypress/e2e/acessibilidade.cy.js` | Testes de acessibilidade (WCAG 2.1 AA) com cypress-axe |
| `cypress/support/commands.js` | Comandos customizados (`clearCourses`, `fillCourse`, `clickCadastrar`, etc.) |
| `cypress/fixtures/` | Dados de teste reutilizáveis (course-online.json, course-defaults.json) |
| `cypress.config.js` | Configuração do Cypress (baseUrl, evidências, vídeo, Mochawesome) |
| `executar_testes.py` | Testes originais em Playwright (mantido como referência) |
| `.github/workflows/cypress.yml` | Pipeline CI/CD com GitHub Actions |
...
- **Relatório HTML:** `evidencias/report/` — relatório Mochawesome com status de cada teste, screenshots embarcados e tempo de execução

As evidências estão disponíveis no Google Drive:
...
Esta seção documenta as principais escolhas de arquitetura e ferramentas feitas durante o desafio, com as respectivas justificativas.

### Por que os testes negativos falham?

Os 11 testes que falham são **intencionais** — cada falha é uma asserção real (`expect()` / `should()`) que comprova um bug na aplicação. Esta abordagem foi escolhida porque:

1. **Evidência automática:** Quando o bug for corrigido, o teste passa automaticamente — sem necessidade de reescrever.
2. **Rastreabilidade:** Cada teste que falha está vinculado a um BUG-ID específico no registro de bugs.
3. **Screenshots antes da falha:** Usamos o padrão `.then()` para capturar screenshot antes da asserção que vai falhar, garantindo evidência visual do estado da aplicação.

### Por que `cy.intercept` ao invés de `cy.wait(ms)`?

Esperas fixas (`cy.wait(2000)`) são a principal causa de testes flaky. Em vez disso:

| Cenário | Estratégia utilizada |
|---|---|
| Navegação entre páginas | `cy.contains('Lista de cursos').should('be.visible')` |
| Requisições DELETE | `cy.intercept('DELETE', '**/*').as('req')` + `cy.wait('@req')` |
| Carregamento de conteúdo | `cy.get('.q-card').should('exist')` |
| Fechamento de dropdown | `cy.get('.q-menu').should('not.exist')` |

### Por que Mochawesome?

Relatórios HTML gerados automaticamente permitem que stakeholders não-técnicos visualizem os resultados sem instalar o Cypress. O relatório inclui:
- Status de cada teste (pass/fail) com tempo de execução
- Screenshots embarcados nas falhas
- Filtros por status para análise rápida

### Por que testes de acessibilidade?

Qualidade vai além de funcionalidade. Os testes com `cypress-axe` validam conformidade com WCAG 2.1 AA, identificando barreiras para usuários com deficiência. Mesmo que a aplicação falhe (provável — muitos frameworks não configuram aria-labels corretamente por padrão), documentar as violações demonstra visão holística de QA.

### CI/CD com GitHub Actions

O pipeline em `.github/workflows/cypress.yml` executa os testes automaticamente a cada push/PR, usando `continue-on-error: true` para não falhar o workflow (já que 11 testes falham por design). As evidências (screenshots, vídeos, relatório) são publicadas como artefatos do workflow.

---

## 7. Sugestões para o Time de Desenvolvimento

Com base nos bugs encontrados e na análise do código-fonte, seguem sugestões priorizadas para melhorar a aplicação:

### Correções de segurança (prioridade máxima)

1. **Corrigir XSS (BUG-006):** Substituir `innerHTML` por `textContent` (ou `v-text` no Vue) ao renderizar o nome do curso. Considerar uso de DOMPurify para sanitizar inputs.

### Correções funcionais

2. **Implementar validação de formulário (BUG-002):** Usar `QInput` rules do Quasar para validação reativa dos campos obrigatórios. Campos mínimos: nome, descrição, instrutor, datas, vagas, tipo.
3. **Corrigir exclusão de curso (BUG-001):** Substituir o endpoint hardcoded `/test-api/courses/1` pelo ID real do curso. Implementar `localStorage.removeItem` ou filtro do array após exclusão.
4. **Validar coerência de datas (BUG-003):** Adicionar validação `endDate >= startDate` no submit do formulário.
5. **Rejeitar vagas negativas/zero (BUG-004):** Adicionar `min="1"` no input e validação no submit.

### Melhorias de robustez

6. **Debounce no botão cadastrar (BUG-008):** Desabilitar o botão durante o processamento ou usar `loading` state do QButton.
7. **Limitar campos de texto (BUG-007):** Adicionar `maxlength` nos inputs (ex: nome=200, descrição=2000).
8. **Validar URL de imagem (BUG-005):** Regex simples para formato http(s):// ou usar `type="url"` no input.

### Melhorias de arquitetura

9. **Acessibilidade:** Revisar aria-labels, contraste de cores e navegação por teclado para conformidade WCAG 2.1 AA.
